...
export function fetchModels() {
  sendRpc("models.list", {}).then(function (res) {
    ...
    var found = S.models.find(function (m) { return m.id === saved; });
    if (found) {
      ...
      if (S.modelComboLabel) S.modelComboLabel.textContent = S.models[0].displayName || S.models[0].id;
...
export function openModelDropdown() {
  ...
  S.setModelIdx(-1);
  renderModelList("");
  requestAnimationFrame(function () { if (S.modelSearchInput) S.modelSearchInput.focus(); });
}
...
export function bindModelComboEvents() {
  ...
  S.modelComboBtn.addEventListener("click", function () {
  ...
  S.modelSearchInput.addEventListener("input", function () {
    S.setModelIdx(-1);
    renderModelList(S.modelSearchInput.value.trim());
  });

  S.modelSearchInput.addEventListener("keydown", function (e) {
    var items = S.modelDropdownList.querySelectorAll(".model-dropdown-item");
    if (e.key === "ArrowDown") {
      e.preventDefault();
