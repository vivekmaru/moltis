...
import { sendRpc, formatTokens, parseErrorMessage, updateCountdown } from "./helpers.js";
...
export function highlightAndScroll(msgEls, messageIndex, query) {
  ...
  setTimeout(function () {
    if (!S.chatMsgBox) return;
    S.chatMsgBox.querySelectorAll("mark.search-term-highlight").forEach(function (m) {
      var parent = m.parentNode;
      parent.replaceChild(document.createTextNode(m.textContent), m);
      parent.normalize();
    });
    S.chatMsgBox.querySelectorAll(".search-highlight-msg").forEach(function (el) {
...
export function updateTokenBar() {
  ...
  var text =
    formatTokens(S.sessionTokens.input) + " in / " +
    formatTokens(S.sessionTokens.output) + " out \u00b7 " +
