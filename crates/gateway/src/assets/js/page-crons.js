...
registerPage("/crons", function initCrons(container) {
  ...
  function openCronModal(existingJob) {
    ...
    function updateSchedParams() {
      ...
      else { schedParams.appendChild(schedCronInput); schedParams.appendChild(schedTzInput); }
    }
    ...
    if (isEdit) {
      ...
      if (sc.kind === "at" && sc.atMs) schedAtInput.value = new Date(sc.atMs).toISOString().slice(0, 16);
      else if (sc.kind === "every" && sc.everyMs) schedEveryInput.value = Math.round(sc.everyMs / 1000);
      else if (sc.kind === "cron") { schedCronInput.value = sc.expr || ""; schedTzInput.value = sc.tz || ""; }
    ...
    saveBtn.addEventListener("click", function () {
      ...
      var schedule, kind = schedSelect.value;
      if (kind === "at") {
        var ts = new Date(schedAtInput.value).getTime();
        if (isNaN(ts)) { schedAtInput.classList.add("field-error"); return; }
        schedule = { kind: "at", atMs: ts };
      } else if (kind === "every") {
        var secs = parseInt(schedEveryInput.value, 10);
        if (isNaN(secs) || secs <= 0) { schedEveryInput.classList.add("field-error"); return; }
        schedule = { kind: "every", everyMs: secs * 1000 };
      } else {
        var expr = schedCronInput.value.trim();
        if (!expr) { schedCronInput.classList.add("field-error"); return; }
        schedule = { kind: "cron", expr: expr };
        var tz = schedTzInput.value.trim();
        if (tz) schedule.tz = tz;
      }
      var msgText = payloadTextInput.value.trim();
      if (!msgText) { payloadTextInput.classList.add("field-error"); return; }
      var payload = payloadSelect.value === "systemEvent"
      ...
      var rpcParams = isEdit
        ? { id: existingJob.id, patch: { name: name, schedule: schedule, payload: payload, sessionTarget: targetSelect.value, deleteAfterRun: deleteAfterCheck.checked, enabled: enabledCheck.checked } }
        : { name: name, schedule: schedule, payload: payload, sessionTarget: targetSelect.value, deleteAfterRun: deleteAfterCheck.checked, enabled: enabledCheck.checked };
      sendRpc(rpcMethod, rpcParams).then(function (res) {
        if (res && res.ok) { closeProviderModal(); loadJobs(); loadStatus(); }
        else { saveBtn.disabled = false; saveBtn.textContent = isEdit ? "Update" : "Create"; }
      });
    });
  ...
  S.$("cronAddBtn").addEventListener("click", function () { openCronModal(null); });
  S.$("cronRefreshBtn").addEventListener("click", function () { loadJobs(); loadStatus(); });
  loadStatus();
  loadJobs();
});
